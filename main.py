...
df = pd.read_csv('NYC_Jobs.csv')
eap = df[df['Job Category'] == 'Engineering, Architecture, & Planning']['Career Level'].value_counts()
print(eap)
eap.plot(kind='pie')
...
df = pd.read_csv('NYC_Jobs.csv')
eap_agency = df[df['Job Category'] == 'Engineering, Architecture, & Planning']['Agency'].value_counts()
print(eap_agency)
...
df = pd.read_csv('NYC_Jobs.csv')
eap_agency = df[df['Agency'] == 'DEPT OF ENVIRONMENT PROTECTION']['Business Title'].value_counts()
